<x-app-layout>
    ...
    <div class="w-3/4 p-6 mt-12 mx-auto bg-white shadow-lg rounded-sm border border-gray-200">
        <header class="px-5 py-4 mb-5">
            <a href="{{ route('admin.adduser') }}" class="bg-teal-500 p-2 mx-6 hover:bg-teal-600 rounded-md text-white text-center shadow-xl shadow-bg-blue-700 text-sm font-bold">
                    ...
                    <tbody class="text-sm divide-y divide-gray-100">
                        @foreach ($users as $user)
                            <tr>
                                ...
                                <td class="p-2 whitespace-nowrap">
                                    @if ($user->role == 'admin')
                                        <div class="text-lg text-center capitalize bg-green-600 text-white rounded-md">Administrador</div>
                                    @else
                                        <div class="text-lg text-center capitalize bg-yellow-600 text-white rounded-md">{{ $user->role }}</div>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
