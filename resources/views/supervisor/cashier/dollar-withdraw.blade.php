<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Retiro de dolares') }}
        </h2>
    </x-slot>

    <div class="w-3/4 p-6 mt-12 mx-auto bg-white shadow-lg rounded-sm border border-gray-200">
        <div class="mx-auto w-full max-w-[550px]">
            <h2 class="text-center mb-5 text-xl font-bold">Ingrese cantidad y datos de supervisor</h2>
            ...
            <form action="{{ route('withdraw.store') }}" method="POST">
                @csrf
                <input type="hidden" name="type" value="dollar">
                <div class="mb-5">
                    <label for="amount" class="mb-3 block text-base font-medium text-[#07074D]">
                        Cantidad de retiro (En dólares)
                    </label>
                    <input
                        autofocus
                        type="number"
                    ...
                    @if (session('amountError'))
                        <p class="text-red-500">{{ session('amountError') }}</p>
                    @endif
                </div>
